...
- (void)viewDidLoad {
    ...
    _arrayData =@[@"share小格", @"share小明", @"share小李", @"share小坤", @"share小雪"];
    ...
    [_tableView registerClass:[GuanzhuTableViewCell class] forCellReuseIdentifier:@"guanzhu"];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        GuanzhuTableViewCell *cell = [_tableView dequeueReusableCellWithIdentifier:@"guanzhu" forIndexPath:indexPath];
        ...
        cell.btn.frame = CGRectMake(10, 10, 100, 40); // 请根据实际布局设置按钮的 frame
            
        cell.btn.layer.cornerRadius = cell.btn.bounds.size.width / 5.0;
        cell.btn.layer.masksToBounds = YES;
        cell.btn.layer.borderWidth = 1.0;
        cell.btn.layer.borderColor = newColor.CGColor;

        CALayer *separatorLayer = [CALayer layer];
        separatorLayer.frame = CGRectMake(0, 80, self.view.frame.size.width - 10, 1); // 修改宽度，减去左右边距

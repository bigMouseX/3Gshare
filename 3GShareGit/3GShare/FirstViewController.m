...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.section == 0) {
        ScrollerTableViewCell *cell = [_tableView dequeueReusableCellWithIdentifier:@"scroller" forIndexPath:indexPath];
        return cell;
    } else {
        ShareTableViewCell *cell = [_tableView dequeueReusableCellWithIdentifier:@"share" forIndexPath:indexPath];
        cell.titleLabel.text = _arrayData[indexPath.section - 1][0];
        cell.labelFirst.text = _arrayData[indexPath.section - 1][1];
        cell.labelSecond.text = _arrayData[indexPath.section - 1][2];
        cell.labelThird.text = _arrayData[indexPath.section - 1][3];
        cell.backgroundColor = [UIColor whiteColor];//解决办法：初始化时随意调用下contentview，比如背景色
        // 通过调用颜色来实现点击事件，与懒加载有关
        
        NSString *listName = [NSString stringWithFormat:@"list_img%ld.png", indexPath.section];

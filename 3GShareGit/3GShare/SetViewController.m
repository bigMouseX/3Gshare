...
- (void)viewDidLoad {
    ...
    _arrayData =@[@"基本资料", @"修改密码", @"消息设置", @"关于SHARE", @"清理缓存"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row == 0) {
        ...
        BasicViewController *t = [[BasicViewController alloc] init];
        // 重新将返回页面的视图控制器添加到导航栈中
        [self.navigationController pushViewController:t animated:YES];
    }
    if (indexPath.row == 1) {
        ChangeViewController *t = [[ChangeViewController alloc] init];
        // 重新将返回页面的视图控制器添加到导航栈中
        [self.navigationController pushViewController:t animated:YES];
    }
    if (indexPath.row == 2) {
        //如何保存pop的界面，这里很重要，只要对该谈出的视图控制器进行强引用，然后只创建一次，类似于单例模式
        if (!_guanzhu)
        _guanzhu = [[XiaoxiSetViewController alloc] init];
//        [self.navigationController pushViewController:t animated:YES];
        // 重新将返回页面的视图控制器添加到导航栈中
    ...
    if (indexPath.row == 4) {
        self.alert = [UIAlertController alertControllerWithTitle:@"通知" message:@"缓存已清除" preferredStyle:UIAlertControllerStyleAlert];
